...
int readTheStats(const char filename[],Dataset& data){
	if(filename == NULL){
		return -1;
}
...
int writeTheStats(const char statName[], const Statistics& stats){
...
int readFile(char*theFile){
	...
	if(checkDistance(theDistance,counter,arrayOfDistances)==false){
		...
		Dataset data = {0};
		Statistics stat = {0};
		readTheStats("data.txt", data);
		computeStatistics(data, stat);
		writeTheStats("out.txt", stat);
		break;
	}
	else{
